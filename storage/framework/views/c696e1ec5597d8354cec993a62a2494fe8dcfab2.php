<script>
    $(function() {
        var table = $('.yajra-datatable').DataTable({
            ...
            ajax: {
                url: "<?php echo e(url('admin/search_package')); ?>",
                type: 'POSt',
                data: function(d) {
                    d.name = $('#name').val();
    ...
    function delete_data(id) {
        swal({
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: 'get',
                    url: "<?php echo e(url('admin/package_delete')); ?>/" + id,
    ...
    $('#save_info').off().on('submit', function(event) {
        event.preventDefault();
        $.ajax({
            url: "<?php echo e(url('admin/package_create')); ?>",
            type: "POST",
    ...
    function edit(id) {
        ...
        $.ajax({
            url: "<?php echo e(url('admin/package_edit')); ?>/" + id,
            type: "GET",
            success: function(response) {
                if (response) {
    ...
    $('#update_form').off().on('submit', function(event) {
        event.preventDefault();
        $.ajax({
            url: "<?php echo e(url('admin/package_update')); ?>",
            type: "POST",
            data: $(this).serializeArray(),
            success: function(response) {
                if (response.success) {
...
</script><?php /**PATH C:\xampp\htdocs\tilmedia_next_decade_mir_dohs\resources\views/backend/package/package_js.blade.php ENDPATH**/ ?>