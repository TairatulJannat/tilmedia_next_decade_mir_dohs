<script>
    $(function() {
        var table = $('.yajra-datatable').DataTable({
            ...
            ajax: {
                url: "<?php echo e(url('admin/search_admin')); ?>",
                type: 'POST',
                data: function(d) {
    ...
    function delete_data(id) {
        swal({
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: 'get',
                    url: "<?php echo e(url('admin/delete_admin')); ?>/" + id,
    ...
    function edit(id) {
        ...
        $.ajax({
            url: "<?php echo e(url('admin/edit_admin')); ?>/" + id,
            type: "GET",
            success: function(response) {
                if (response) {
...
var editroles = '<option value="' + response.admin.role.id +'" selected >' + response.admin.role.name + '</option>';
   ...
   $.each(response.roles, function(key, roleList) {
    ...
    $('#update_form').off().on('submit', function(event) {
        event.preventDefault();
        $.ajax({
            url: "<?php echo e(url('admin/update_admin')); ?>",
            type: "POST",
            data: $(this).serializeArray(),
            success: function(response) {
                if (response) {
...
</script><?php /**PATH C:\xampp\htdocs\tilmedia_next_decade_mir_dohs\resources\views/backend/admin/admin_js.blade.php ENDPATH**/ ?>